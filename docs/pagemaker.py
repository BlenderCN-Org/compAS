...
def module_doc(p):
    ...
    with open(filepath, 'w+') as fp:
        pdoc = p.__doc__ or '<docstring missing>'
        line = PACAKGE_PAGE.format(p.__name__, pdoc)
        fp.write(line)
    if packages:
    ...
    for m in modules:
        ...
        with open(filepath, 'a') as fp:
            ...
            mdoc = m.__doc__
            fp.write(SUBMODULE_SECTION.format(mname, mdoc))
...
def package_doc(package):
    ...
    with open(filepath, 'w+') as fp:
        pdoc = package.__doc__ or '<...docstring missing...>'
        fp.write(PACAKGE_PAGE.format(package.__name__, pdoc))
    if packages:
        with open(filepath, 'a') as fp:
            fp.write(SUBPACKAGES_SECTION)
            for n in packages:
                fp.write('   ' + objects[n].__name__.replace('.', '-') + '\n')
    submodules = [n for n in modules if n not in packages]
    if submodules:
        with open(filepath, 'a') as fp:
            fp.write('\n.. rubric:: Modules\n\n')
            for n in submodules:
        ...
        for n in submodules:
            ...
            # what about module-level variables?
            with open(filepath, 'a') as fp:
                module = objects[n]
                mname = module.__name__.split('.')[-1]
                mdoc = module.__doc__ or ''
                fp.write(SUBMODULE_SECTION.format(mname, mdoc))
                for o in clsss:
...
def function_doc(f):
    ...
    filepath = 'pages/api/' + name.replace('.', '-') + '.rst'
    with open(filepath, 'w+') as fp:
        fp.write(FUNCTION_PAGE.format(fname, name))
