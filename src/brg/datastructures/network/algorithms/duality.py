...
from brg.geometry.functions import is_ccw
...
def find_network_faces(network):
    del network.face
    network.face = {}
    ...
    if leaves:
        u = sorted([(leaf, network.vertex[leaf]) for leaf in leaves], key=lambda x: (x[1]['y'], x[1]['x']))[0][0]
    else:
